...
def num_examples_experiment(plot_name, fig_num, X_train, Y_train, X_val, Y_val, num_examples_arr, **kwargs):
    ...
    for num in num_examples_arr:
        ...
        if fig_num == 2:
            # Is CIFAR, save.
            pickle.dump(clf, open("cifar/CIFAR.model", 'wb'))
...
def main_q3():
    ...
    for i, acc in enumerate(val_acc_arr):
        if acc == max_acc:
            max_i = i
    best_c = c_arr[max_i]
    print(f"BEST C, coarse search: {best_c}")
    ...
    for i, acc in enumerate(val_acc_arr):
        if acc == max_acc:
            max_i = i
    best_c = to_check[max_i]
    print(f"BEST C, fine search: {best_c}")
...
def k_fold_cross_validation(X_train, Y_train, k, n, C):
    ...
    for i in range(0, k):
        if i == k - 1:
            ...
            valid_X = X_train[part_size:]
            valid_Y = Y_train[part_size:]
            cut_X = X_train[0: part_size]
            cut_Y = Y_train[0: part_size]
        else:
            valid_X = X_train[i * part_size: (i + 1) * part_size]
            valid_Y = Y_train[i * part_size: (i + 1) * part_size]
            cut_X = np.vstack((X_train[0: i * part_size], X_train[(i + 1) * part_size:]))
            cut_Y = np.hstack((Y_train[0: i * part_size], Y_train[(i + 1) * part_size:]))
    ...
    print(f"C={C}, AVG TRAIN: {sum(train_arr) / len(train_arr)}, AVG VAL: {sum(val_arr) / len(val_arr)}***")
    return sum(train_arr) / len(train_arr), sum(val_arr) / len(val_arr)


def main_q4():
    curr = main_q1()[1]
    c_arr = [10000, 1000, 100, 10, 1.0]
    train_acc_arr, val_acc_arr = [], []
    for c in c_arr:
    ...
    plt.ylabel("Accuracy")
    plt.xscale("log")
    plt.title("Coarse search for C values for spam.")
    plt.legend()
    ...
    train_acc_arr = []
    val_acc_arr = []
    to_check = sorted([v for v in to_check if v >= 0])
    for c in to_check:
        train_acc, val_acc = k_fold_cross_validation(curr["training_data"],
                                                     curr["training_labels"], 5, len(curr["training_data"]),
                                                     C=c)
